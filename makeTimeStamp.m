...
if size(tmp,1)==2
    if ((sum(event{tmp(1,1)}(1:2)=='SM')==2 || sum(event{tmp(1,1)}(1:2)=='sM')==2 ||...
            sum(event{tmp(1,1)}(1:3)=='S M')==3 || sum(event{tmp(1,1)}(1:3)=='s M')==3) && ...
            (sum(event{tmp(2,1)}(1:2)=='EM')==2 || sum(event{tmp(2,1)}(1:2)=='eM')==2 ||...
            sum(event{tmp(2,1)}(1:3)=='E M')==3 || sum(event{tmp(2,1)}(1:3)=='e M')==3)) ...
...
elseif size(tmp,1)==1
    if event{tmp(1,1)}(1)=='S' || event{tmp(1,1)}(1)=='s'
        MODInfo.name = 'MOD';
        MODInfo.startTime = tmp(1,2);
        MODInfo.endTime = [];
        fprintf('Warning: MOD name is not right. check manually! \n')
...
if size(tmp,1)==2
    if ((sum(event{tmp(1,1)}(1:2)=='SH')==2 || sum(event{tmp(1,1)}(1:2)=='sH')==2 || ...
            sum(event{tmp(1,1)}(1:3)=='S H')==3 || sum(event{tmp(1,1)}(1:3)=='s H')==3) && ...
            (sum(event{tmp(2,1)}(1:2)=='EH')==2 || sum(event{tmp(2,1)}(1:2)=='eH')==2 ||...
            sum(event{tmp(2,1)}(1:3)=='E H')==3 || sum(event{tmp(2,1)}(1:3)=='e H')==3)) ...
            && tmp(2,2)-tmp(1,2)<500
...
elseif size(tmp,1)==1
    if (event{tmp(1,1)}(1)=='S' || event{tmp(1,1)}(1)=='s') && ...
            (strcmp(event{find(eventIndex{10},1)},'SMOD') || strcmp(event{find(eventIndex{10},1)},'S MOD')||...
            strcmp(event{find(eventIndex{10},1)},'sMOD') || strcmp(event{find(eventIndex{10},1)},'s MOD'))
        HVLT2Info.name = 'HVLT2';
        HVLT2Info.startTime = tmp(1,2);
...
if size(tmp,1)==2
    if ((sum(event{tmp(1,1)}(1:2)=='SD')==2 || sum(event{tmp(1,1)}(1:2)=='sD')==2 || ...
            sum(event{tmp(1,1)}(1:3)=='S D')==3 || sum(event{tmp(1,1)}(1:3)=='s D')==3) && ...
            (sum(event{tmp(2,1)}(1:2)=='ED')==2 || sum(event{tmp(2,1)}(1:2)=='eD')==2 || ...
            sum(event{tmp(2,1)}(1:3)=='E D')==3 || sum(event{tmp(2,1)}(1:3)=='e D')==3)) ...
            && tmp(2,2)-tmp(1,2)<500
        % if naming seems correct, and the duration is not too long (>500sec)
        DSInfo.name = 'DS';
...
elseif size(tmp,1)==1
    if (event{tmp(1,1)}(1)=='S' || event{tmp(1,1)}(1)=='s') && ...
            (strcmp(event{find(eventIndex{9},1)},'SHVLT delay/recog')||...
            strcmp(event{find(eventIndex{9},1)},'S HVLT delay/recog')||...
            strcmp(event{find(eventIndex{9},1)},'sHVLT delay/recog')||...
            strcmp(event{find(eventIndex{9},1)},'s HVLT delay/recog'))
        DSInfo.name = 'DS';
        DSInfo.startTime = tmp(1,2);
...
if size(tmp,1)==2
    if ((sum(event{tmp(1,1)}(1:2)=='SW')==2 || sum(event{tmp(1,1)}(1:2)=='sW')==2 ||...
        sum(event{tmp(1,1)}(1:3)=='S W')==3 || sum(event{tmp(1,1)}(1:3)=='s W')==3) && ...
            (sum(event{tmp(2,1)}(1:2)=='EW')==2 || sum(event{tmp(2,1)}(1:2)=='eW')==2 ||...
            sum(event{tmp(2,1)}(1:3)=='E W')==3 || sum(event{tmp(2,1)}(1:3)=='e W')==3)) ...
            && tmp(2,2)-tmp(1,2)<500
        % if naming seems correct, and the duration is not too long (>500sec)
        WTARInfo.name = 'WTAR';
...
elseif size(tmp,1)==1
    if (event{tmp(1,1)}(1)=='S'||event{tmp(1,1)}(1)=='s') && ...
            (strcmp(event{find(eventIndex{8},1)},'SDS')||strcmp(event{find(eventIndex{8},1)},'S DS')||...
            strcmp(event{find(eventIndex{8},1)},'sDS')||strcmp(event{find(eventIndex{8},1)},'s DS'))
        WTARInfo.name = 'WTAR';
        WTARInfo.startTime = tmp(1,2);
...
if size(tmp,1)==2
    if ((sum(event{tmp(1,1)}(1:2)=='ST')==2 || sum(event{tmp(1,1)}(1:2)=='sT')==2 ||...
            sum(event{tmp(1,1)}(1:3)=='S T')==3 || sum(event{tmp(1,1)}(1:3)=='s T')==3) && ...
            (sum(event{tmp(2,1)}(1:2)=='ET')==2 || sum(event{tmp(2,1)}(1:2)=='eT')==2 ||...
            sum(event{tmp(2,1)}(1:3)=='E T')==3 || sum(event{tmp(2,1)}(1:3)=='e T')==3)) ...
            && tmp(2,2)-tmp(1,2)<500
...
elseif size(tmp,1)==1
    if (event{tmp(1,1)}(1)=='S'||event{tmp(1,1)}(1)=='s') && ...
            (strcmp(event{find(eventIndex{7},1)},'SWTAR')||strcmp(event{find(eventIndex{7},1)},'S WTAR')||...
            strcmp(event{find(eventIndex{7},1)},'sWTAR')||strcmp(event{find(eventIndex{7},1)},'s WTAR'))
        TMInfo.name = 'TM';
        TMInfo.startTime = tmp(1,2);
...
if size(tmp,1)==2
    if ((sum(event{tmp(1,1)}(1:2)=='SC')==2 || sum(event{tmp(1,1)}(1:2)=='sC')==2 || ...
            sum(event{tmp(1,1)}(1:3)=='S C')==3 || sum(event{tmp(1,1)}(1:3)=='s C')==3) && ...
            (sum(event{tmp(2,1)}(1:2)=='EC')==2 || sum(event{tmp(2,1)}(1:2)=='eC')==2 || ...
            sum(event{tmp(2,1)}(1:3)=='E C')==3 || sum(event{tmp(2,1)}(1:3)=='e C')==3)) ...
            && tmp(2,2)-tmp(1,2)<500
        % if naming seems correct, and the duration is not too long (>500sec)
...
elseif size(tmp,1)==1
    if (event{tmp(1,1)}(1)=='S' ||event{tmp(1,1)}(1)=='s') &&...
            (strcmp(event{find(eventIndex{6},1)},'STM')||strcmp(event{find(eventIndex{6},1)},'S TM')||...
            strcmp(event{find(eventIndex{6},1)},'sTM')||strcmp(event{find(eventIndex{6},1)},'s TM'))
        COWATInfo.name = 'COWAT';
        COWATInfo.startTime = tmp(1,2);
...
if size(tmp,1)==2
    if ((sum(event{tmp(1,1)}(1:2)=='SS')==2 || sum(event{tmp(1,1)}(1:2)=='sS')==2 ||...
            sum(event{tmp(1,1)}(1:3)=='S S')==3 || sum(event{tmp(1,1)}(1:3)=='s S')==3) && ...
            (sum(event{tmp(2,1)}(1:2)=='ES')==2 || sum(event{tmp(2,1)}(1:2)=='eS')==2 ||...
            sum(event{tmp(2,1)}(1:3)=='E S')==3 || sum(event{tmp(2,1)}(1:3)=='e S')==3)) ...
            && tmp(2,2)-tmp(1,2)<500
...
elseif size(tmp,1)==1
    if (event{tmp(1,1)}(1)=='S'||event{tmp(1,1)}(1)=='s') &&...
            (strcmp(event{find(eventIndex{5},1)},'SCOWAT')||strcmp(event{find(eventIndex{5},1)},'S COWAT')||...
            strcmp(event{find(eventIndex{5},1)},'sCOWAT')||strcmp(event{find(eventIndex{5},1)},'s COWAT'))
        SDMInfo.name = 'SDM';
        SDMInfo.startTime = tmp(1,2);
...
if size(tmp,1)>=2
    ...
    elseif (event{tmp(1,1)}(1)=='S'||event{tmp(1,1)}(1)=='s') && ...
            (strcmp(event{find(eventIndex{4},1)},'SSDM')||strcmp(event{find(eventIndex{4},1)},'S SDM')||...
            strcmp(event{find(eventIndex{4},1)},'sSDM')||strcmp(event{find(eventIndex{4},1)},'s SDM'))
        STInfo.name = 'ST';
        STInfo.startTime = tmp(1,2);
...
elseif size(tmp,1)==2
    if ((event{tmp(1,1)}(1)=='S'||event{tmp(1,1)}(1)=='s') &&...
            (event{tmp(2,1)}(1)=='E'||event{tmp(2,1)}(1)=='e')) && tmp(2,2)-tmp(1,2)<300
        % if naming seems correct (at least for HVLT1), and the duration is not too long (>300sec)
        HVLT1Info.name = 'HVLT1';
        HVLT1Info.startTime = tmp(1,2);
        HVLT1Info.endTime = tmp(2,2);
    elseif (event{tmp(1,1)}(1)=='S'|| event{tmp(1,1)}(1)=='s') && ...
            (event{tmp(2,1)}(1)=='S'||event{tmp(2,1)}(1)=='s') % only the start of HVLT1 & HVLT-recall have been recorded
...
if size(tmp,1)<2 % if either only start or end was recorded
   if event{tmp(1,1)}(1)=='S' || event{tmp(1,1)}(1)=='s' % only start of the baseline was recorded
       baselineInfo.name = 'Baseline';
       baselineInfo.startTime = tmp(1,2);
       baselineInfo.endTime = tmp(1,2)+600; % 10 min of baseline
   elseif event{tmp(1,1)}(1)=='E' || event{tmp(1,1)}(1)=='e'% only start of the baseline was recorded
