function elec = read_asa_elc_AI(fn)
...
%
% $Id: read_asa_elc.m 8962 2013-12-05 07:48:46Z roboos $
...
Npnt = read_asa_AI(fn, 'NumberPositions=', '%d');
Ntri = read_asa_AI(fn, 'NumberPolygons=', '%d');
Unit = read_asa_AI(fn, 'UnitPosition', '%s', 1);
pnt  = read_asa_AI(fn, 'Positions', '%f', Npnt, ':');
prj  = read_asa_AI(fn, 'Positions2D', '%f', Npnt, ':'); % only in newer files
tri  = read_asa_AI(fn, 'Polygons', '%d', Ntri);
lab  = read_asa_AI(fn, 'Labels', '%s', Npnt);
ref  = read_asa_AI(fn, 'ReferenceChannel', '%s', 1); % only in newer files
